...
if (_action == 0) then {								//Mark the group
		if (count MCC_groupGenGroupArray > 0) then {
			...
			_unitsCount = [group _leader] call MCC_fnc_countGroup;			//Let's count the units' type
			_unitsCount = [_unitsCount select 0,_unitsCount select 1,_unitsCount select 2,_unitsCount select 3,_unitsCount select 4];
	...
	if (MCC_groupGenGroupStatus == 0) then 	//West
		{	
			...
			if ((side (leader _x) == west) && !(isPlayer leader _x)) then	
				{
					_unitsCount = [_x] call MCC_fnc_countGroup;
