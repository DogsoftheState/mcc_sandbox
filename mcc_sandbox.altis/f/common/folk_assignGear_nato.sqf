...
_lightUniform = "U_B_CombatUniform_mcam_tshirt";
_baseHelmet = "H_HelmetB";
_lightHelmet = "H_HelmetB_light";
...
_backpack = {
	...
	switch (_typeofBackPack) do
	{
		...
		case "smed":
		{
			...
			(unitBackpack _unit) addMagazineCargoGlobal [_SFriflemag, 4];
			(unitBackpack _unit) addItemCargoGlobal [_medkit,1];
			(unitBackpack _unit) addItemCargoGlobal [_firstaid,15];
		};
...
// LOADOUT: COMMANDER
	case "co":
	{
		...
		if (isClass (configFile >> "cfgWeapons" >> "MCC_Console")) then {_unit linkItem  "MCC_Console"} else {_unit linkItem "B_UavTerminal"}; 
		["g"] call _backpack;
	};
...
// LOADOUT: RIFLEMAN (AT)	
	...
	{
		{_unit addmagazine _carbinemag} foreach [1,2,3,4,5];
		{_unit addmagazine _carbinemag_tr} foreach [1];
		_unit addweapon _carbine;
